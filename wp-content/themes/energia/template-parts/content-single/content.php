 ...
 * @package Energia
 */
$post_tags_on = energia_get_opt( 'post_tags_on', true );
$post_navigation_on = energia_get_opt( 'post_navigation_on', false );
$post_author_info_on = energia_get_opt( 'post_author_info_on', false );
$post_social_share_on = energia_get_opt( 'post_social_share_on', false );
$post_feature_image_on = energia_get_opt( 'post_feature_image_on', true );
$post_title_on = energia_get_opt('post_title_on', false);
?>
<article id="post-<?php the_ID(); ?>" <?php post_class('single-hentry'); ?>>
    <?php if (has_post_format('gallery')) : ?>
        <div class="entry-featured">
            <?php
            $light_box = energia_get_post_format_value('post-gallery-lightbox', '0');
            $gallery_list = explode(',', energia_get_post_format_value('post-gallery-images', ''));
    ...
    <?php elseif (has_post_format('quote')) : ?>
        <div class="entry-featured">
            <?php
                $quote_text = energia_get_post_format_value('post-quote-cite', '');
    ...
    <?php elseif (has_post_format('link')) : ?>
        <div class="entry-featured">
            <?php
                $link_pf = energia_get_post_format_value('post-link-url', '#');
    ...
    <?php elseif (has_post_format('video')) : ?>
        <div class="entry-featured">
            <div class="entry-video featured-active">
                <?php
                $video_url = energia_get_post_format_value('post-video-url', '#');
                $video_file = energia_get_post_format_value('post-video-file', '');
                $video_html = energia_get_post_format_value('post-video-html', '');
                $video = '';
                if (!empty($video_url)) {
                    global $wp_embed;
    ...
    <?php elseif (has_post_format('audio')) : ?>
        <div class="entry-featured">
            <?php
            $audio_url = energia_get_post_format_value('post-audio-url', '#');
        ...
        <div class="entry-content-top">
            ...
            if ( get_the_tag_list() || $post_social_share_on ){
                ...
                <div class="entry-content-bottom">
                    <?php
                    if($post_tags_on) :
                        energia_entry_tagged_in();
                    endif;
                    if($post_social_share_on) {
                        energia_socials_share_default();
                    }
                    ?>
                </div>
                <?php
