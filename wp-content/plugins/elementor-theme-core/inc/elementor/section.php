...
//		parent::__construct( $data );
	...
	protected function content_template() {
        ...
        let custom_attr = elementor.hooks.applyFilters('etc-custom-section-attr', {}, settings);
        custom_attr = _.isObject(custom_attr)?custom_attr:{};
        let custom_attr_output = '';
        _.each(custom_attr, function (attr_value, attr_name) {
            custom_attr_output += attr_name + '="' + attr_value + '" ';
        });
        ...
        {{{ beforeElementorRow }}}
        <div class="elementor-container elementor-column-gap-{{ settings.gap }} {{ custom_classes.join(' ') }}" {{ custom_attr_output }}>
	...
	public function before_render() {
		...
		$has_background_overlay = in_array( $settings['background_overlay_background'], [
				...
				'gradient'
		                          ...
		                          in_array( $settings['background_overlay_hover_background'], [
			                          'classic',
			                          'gradient'
		                          ], true );
		...
		$custom_attr        = apply_filters( 'etc-custom-section-attr', [], $settings );
		$custom_attr        = is_array( $custom_attr ) ? $custom_attr : [];
		$custom_attr_output = '';
		foreach ( $custom_attr as $attr_name => $attr_value ) {
			$custom_attr_output .= $attr_name . '="' . $attr_value . '" ';
		}
        ...
        <div class="elementor-container elementor-column-gap-<?php echo esc_attr( $settings['gap'] ); ?> <?php echo esc_attr( implode( ' ', $custom_classes ) ) ?>" <?php printf('%s', $custom_attr_output ); ?>>
		<?php if ( ! Plugin::$instance->experiments->is_feature_active( 'e_dom_optimization' ) ) { ?>
	...
	protected function get_html_tag() {
		...
		if ( empty( $html_tag ) ) {
			$html_tag = apply_filters( 'etc-section-html-tag', 'section' );
		}
